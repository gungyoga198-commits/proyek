...
<section class="relative h-72 flex items-center justify-center overflow-hidden">
    ...
    @endphp
    <img src="{{ $heroImage }}" class="absolute inset-0 w-full h-full object-cover" alt="Gallery Banner">
...
<section class="bg-white py-10 px-6">
    <div class="flex justify-center gap-3 flex-wrap" id="filter-tabs">
        ...
        <button onclick="filterGallery('{{ $k }}', this)"
...
<!-- GALLERY GRID -->
<section class="bg-gray-50 pb-24 px-6 md:px-16 {{ $galleries->isEmpty() ? 'pt-16' : '' }}">
    ...
    <div class="max-w-6xl mx-auto columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6" id="gallery-grid">
        ...
        <div class="gallery-item break-inside-avoid group cursor-pointer opacity-0"
             style="animation: fadeInUp 0.6s ease forwards; animation-delay: {{ min($i * 0.08, 0.8) }}s;"
             data-category="{{ $item->kategori }}"
             onclick="openLightbox('{{ asset('storage/' . $item->foto) }}', '{{ addslashes($item->judul) }}')">
            ...
            <div class="relative border-2 border-yellow-500/70 rounded-xl p-1.5
                ...
                <div class="relative overflow-hidden rounded-lg">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4
                        ...
                        <p class="text-white text-sm font-semibold leading-tight">{{ $item->judul }}</p>
                        @if($item->deskripsi)
                        <p class="text-gray-300 text-xs mt-0.5 line-clamp-1">{{ $item->deskripsi }}</p>
                        @endif
                    </div>
                </div>
            </div>
    ...
    {{-- Pesan saat filter kategori tidak punya hasil --}}
