<footer class="bg-gray-200 py-5 px-5">
    <div class="max-w-6xl mx-auto grid md:grid-cols-2 gap-5 text-center">

        <!-- LEFT -->
        <div class="flex flex-col items-center">
            <img src="/images/LogoHotel.png" class="h-24 mb-6">

            <p class="text-sm text-gray-700 leading-relaxed max-w-md">
                Experience the ultimate convenience at our centrally situated Legian hotel,
                a stone's throw away from renowned beaches like Double Six and Padma,
                the airport, delectable dining, lively pubs, premier shopping,
                and Bali's thrilling Waterbom attractions.
            </p>
        </div>

        <!-- RIGHT -->
        <div class="flex flex-col items-center">
            <h3 class="font-semibold mb-4">Get in Touch</h3>

            <div class="space-y-4 text-sm text-gray-700">

                <div>
                    <p class="font-medium">Phone</p>
                    <p class="text-yellow-600">08980563855</p>
                </div>

                <div>
                    <p class="font-medium">Phone</p>
                    <p class="text-yellow-600">08980563855</p>
                </div>

                <div>
                    <p class="font-medium">WhatsApp</p>
                    <p class="text-yellow-600">08980563855</p>
                </div>

                <div>
                    <p class="font-medium">Email</p>
                    <p class="text-yellow-600">
                        reservation@ogagresort.com
                    </p>
                </div>

                <div>
                    <p class="font-medium">Address</p>
                    <p class="text-yellow-600">
                        Jl. Astasura No.1 Peguyangan
                    </p>
                </div>

            </div>
        </div>

    </div>
</footer><?php /**PATH C:\laragon\www\proyek\resources\views/components/footer.blade.php ENDPATH**/ ?>